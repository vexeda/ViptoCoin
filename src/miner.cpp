...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("viptocoin", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("viptocoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("viptocoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("viptocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("viptocoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("viptocoin", "Running viptocoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
