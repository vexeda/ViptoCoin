...
const string strMessageMagic = "Viptocoin Signed Message:\n";
...
//CAmount GetBlockSubsidy(int nBlockHeight, const Consensus::Params& consensusParams)
{
...
CAmount GetProofOfStakeSubsidy()
{
    ...
    if (nBlockHeight >= 2 && nBlockHeight <= 500000) {
     return  239 * COIN;
    }
    if (nBlockHeight >= 500001 && nBlockHeight <= 1000000) {
     return  119 * COIN;
    }
    if (nBlockHeight >= 1000001 && nBlockHeight <= 1500000) {
     return  59* COIN;
    }
    if (nBlockHeight >= 1500001 && nBlockHeight <= 2000000) {
     return  29 * COIN;
    }
    if (nBlockHeight >= 2000001 && nBlockHeight <= 3000000) {
     return  15 * COIN;
    }
    if (nBlockHeight >= 3000001 && nBlockHeight <= 3500000) {
     return  7.5 * COIN;
    }
    if (nBlockHeight >= 1500001 && nBlockHeight <= 4000000) {
     return  3.75 * COIN;
    }
    if (nBlockHeight >= 1500001 && nBlockHeight <= 4500000) {
     return  1.9 * COIN;
    }
    if (nBlockHeight >= 1500001 && nBlockHeight <= 11000000) {
     return  1 * COIN;
    }
    if (nBlockHeight >= 11000001 && nBlockHeight <= 15000000) {
     return  0.001 * COIN;
    }
    if (nBlockHeight >= 15000001 && nBlockHeight <= 18500000) {
     return  0.00001 * COIN;
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Viptocoin also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // Viptocoin also requires low S in sigs
 ...
 //           CAmount blockReward = nFees + GetBlockSubsidy();
  ...
  //          CAmount blockReward = nFees + GetProofOfStakeSubsidy(pindex->nHeight, chainparams.GetConsensus());
