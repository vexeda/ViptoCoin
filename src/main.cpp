...
#if defined(NDEBUG)
#error "viptocoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "viptocoin very Signed Message:\n";
...
int64_t GetBlockValue(int nHeight, bool fProofOfStake, uint64_t nCoinAge)
{
    ...
    int64_t nRewardCoinYear = 786 * CENT; // 50% interest
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from viptocoin
...
{
    RenameThread("viptocoin-scriptch");
...
bool RecalculateECASupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating viptocoin supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating viptocoin supply..."), percent);
        }
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert viptocoin to zECA
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // viptocoin: We use certain sporks during IBD, so check to see if they are
