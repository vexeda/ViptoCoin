...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8339
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8338
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 28339
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18338
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8339
	%{_sbindir}/semanage port -d -p tcp 8338
	%{_sbindir}/semanage port -d -p tcp 28339
	%{_sbindir}/semanage port -d -p tcp 18338
	for selinuxvariant in %{selinux_variants}; do
